...
| 密码 | 6-20 位 |
| 限制 | 用户名唯一，重复注册提示"用户名已被注册" |

操作步骤：点击登录表单底部"还没有账号？注册" → 填写用户名和密码 → 点击"注册"按钮。注册成功后页面显示"注册成功，请登录"，自动切回登录模式并清空表单。
...
| `css/style.css` | 6.5 KB | 全局样式，登录页与看板页布局、配色、拖拽态、响应式 | 翠花 |
...
待蹦蹦（松鼠测试工程师）完成全部测试用例后补充。当前测试报告已覆盖功能、输入校验、安全、边界、性能五大维度，详见：

**[tests/test.md](tests/test.md)**

> 肥波备注：蹦蹦那孩子胆子小但眼尖，Bug 藏不住的。等他测完，这节肥波再接着记。
